...
public interface ReturnReturnTypeRepo extends JpaRepository<ReturnReturnTypeEntity, Long> {
    ...
    @Query("SELECT rrte.returnTypeIdFk.returnTypeId FROM ReturnReturnTypeEntity rrte WHERE rrte.returnIdFk.returnId=:id AND rrte.isActive=true ORDER BY rrte.returnTypeIdFk.returnTypeId")
    List<Long> findByReturnIdFkAndIsActive(Long id);
